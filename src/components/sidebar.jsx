...
import { LayoutDashboard, Receipt, Landmark, Settings, Plus, Copy } from 'lucide-react';

export default function Sidebar({ currentView, setView }) {
  const { store, activeScenario, actions } = useData();

  const handleScenarioChange = (e) => {
    actions.switchScenario(e.target.value);
  };
  ...
  return (
    <div className="w-64 bg-slate-900 text-slate-300 flex flex-col h-screen flex-shrink-0">
      ...
      <div className="p-6 border-b border-slate-800">
        <h1 className="text-white font-bold text-lg tracking-tight">BA Planner <span className="text-blue-500">v8.0</span></h1>
      </div>
      ...
      <div className="p-4 border-b border-slate-800 space-y-3">
        ...
        <button
          ...
          className="w-full flex items-center justify-center gap-2 text-xs bg-slate-800 hover:bg-slate-700 py-2 rounded text-slate-400 transition-colors"
        >
          <Plus size={14} /> New / Clone
        </button>
      </div>
      ...
      <div className="p-4 border-t border-slate-800 text-xs text-slate-600">
        ...
        <span className="text-slate-500 font-mono">
          {new Date(activeScenario.lastUpdated).toLocaleDateString()}
        </span>
      </div>
    </div>
  );
