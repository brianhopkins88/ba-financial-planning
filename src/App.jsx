...
const TopBar = () => {
  ...
  const handleHold = (direction) => {
    ...
    intervalRef.current = setInterval(() => {
        ...
        let step = 1;
        if (elapsed > 2000) step = 3; // After 2s, jump quarters
        if (elapsed > 5000) step = 12; // After 5s, jump years
       ...
       <div className="flex items-center gap-4 bg-slate-100 p-1 rounded-lg border border-slate-200">
          ...
          <div className="flex flex-col items-center w-32 select-none">
              ...
              <span className="text-base font-bold text-blue-700">
                  {format(simulationDate, 'MMM yyyy')}
              </span>
          </div>
...
const AppShell = () => {
  ...
  if (!isLoaded) return <div className="flex h-screen items-center justify-center text-slate-400 animate-pulse">Loading Financial Core v8.6...</div>;
