* * # BA Financial Analysis Application – Architecture Specification (v1.4)
    
      **Date:** December 7th, 2025 **Version:** 1.4
      ...
      ### 1.2 Major Changes in v1.4
    
      - **Property Planner:** Complete implementation of New Construction & Existing Home planner with multi-stage funding, closing cost estimation, and auto-loan sizing.
      - **Engine Core:** (From 1.3) Shifted from Hybrid (Monthly/Annual) to **Strict Monthly Simulation** (420 steps) to ensure calculation precision and eliminate compounding drift.
      ...
      - **Data Store:** `localStorage` key `ba_financial_planner_v1.4_primary_spouse`.
    
      ### 3.2 Schema Updates (v1.4)
    
      - **Property Planner Inputs:** Added support for `purchasePlan` objects in property assets, including closing costs, deposits, and loan sizing.
      - **Funding Logic:** Assets can now have an `inputs.purchasePlan.funding` array to allocate cash-to-close from specific sources.
      - **Income Profile:** (v1.3) Added `birthMonth` (1-12) to person objects for precise first-year proration.
      ...
      - **Data Integrity:** `validateAndRepairImport(json)` function added to Context to sanitize legacy JSON structures on load.
      ...
      **Core Views**
      ...
      4. **Assets & Property:** Liquid assets, Property valuation, New Construction Planner, and Inherited IRA 10-year rule management.
      ...
      **Architecture: Strict Monthly Loop**
      ...
      - **Key Logic:**
        - **Property Planner:** Auto-executes funding transfers (deposits/closing) and loan creation on specific dates.
        - **Taxation:**
      ...
      **Outputs:**
      ...
      - **UI Layer:**
        - **Assets:** Property Planner component updates `asset.inputs` directly.
