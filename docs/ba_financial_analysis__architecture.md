# BA Financial Analysis Application – Architecture Specification (v3.4.0)
...
- **Client-Only Storage:**
  ...
  - Key pattern (current):  
  - `ba_financial_planner_v3.4.0_registry` (v3.4.0 keeps this label for compatibility).
  - Reset clears all known historical keys (v2.0–v3.4.0) before seeding defaults; imports hard-overwrite all stored data after validation/migration.
