...
Version: **0.95** (Supersedes 0.91 – incorporates Social Security proration, Pension auto-start, Progressive IRA taxation, Multi-loan property linking, and Dashboard drill-down features.)
...
- **Role:** Main liquid buffer.
- **Rules:** - Surplus income fills Cash Savings up to `cash_savings_max`.
  - Deficits draw from Cash Savings down to `cash_savings_min` (age-based).
