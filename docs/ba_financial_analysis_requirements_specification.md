...
- **Income Precision:**
  - **Birth Month:** Income configuration must capture the specific **Birth Month** for Brian and Andrea.
  - **Proration:** FICA  payments in the first eligible year are prorated starting from the birth month (or designated start month) rather than the full year.
...
- **Profile Editor Workflow:**
  * The "Active Profile" in the editor is decoupled from the "Active Profile" in the Timeline.
  * **Context Switching:** Users can load any profile (e.g., "Retirement - Lean") into the editor to modify it, regardless of the current simulation date.
  * **Sync Indicator:** The UI displays a "Synced" (Green) or "Unsaved Changes" (Amber) status, comparing the local scratchpad to the saved Master Profile.
  * **Description:** All profiles and scenarios include a text-based `description` field for context (e.g., "Low travel budget for ages 70-80").
* **Liability Linking:**
    * Expense profiles can explicitly **link or unlink** specific liabilities.
    * Example: A "Downsizing" profile might link the new Mortgage but unlink the old HELOC.
    * Unlinked loans remain "Active" in the system (tracking balance/interest) but their monthly payments are **excluded** from the Operating Cash Flow calculation for that period.
...
The simulation logic now uses a **Strict Monthly Engine** (420 steps) to ensure precision and prevent compounding errors.
...
- **State Management:**
  ...
  - **Annual Reset:** Accumulators for the "Detailed Analysis Table" (e.g., Annual Income, Net Flow) are explicitly reset every January (Month 1).
- **Inflation:**
  - Calculated based on `(Total Elapsed Months / 12)`.
  - This ensures smooth, stateless compounding regardless of the simulation step.
- **Taxation Rules (Refined v1.3):**
  * **Employment Income:** Treated as **Net (Take-Home)**. No further tax calculation is applied.
  * **Social Security & Pension:** Treated as **Gross**. The engine applies the effective tax rate (derived from Work Status tiers) to these streams before adding them to Net Cash Flow.
...
- **Detailed Analysis Table:**
  ...
  - **Expense Categories** must strictly match input modules:
    - **Bills**
    - **Mortgage & Impounds**
    - **Home Expenses**
    - **Living Expenses**
    - **Other Liabilities**
    - **Extra Expenses** (One-Offs + Fun Money)
...
- **1.2:** (Current) Strict Monthly Engine (420 steps), Consolidated Cash Flow View, Precise Birth Month Logic, RMD Cash Injection separation.
